...
ir_matrix = zeros(N,37);
Spec_matrix = zeros(floor(N/2)+1,37);

for n = 1:36
    angle = 10*(n-1);        % directions: 0 ~ 350 degrees, 10 degree spacing
    filepath = strcat('C:\Users\root\Documents\00 phd\measurement\210324 Point source directivity\0326 directivity\newsetup\',num2str(angle),'deg');
    cd(filepath)
...
ir_matrix(:,37) = ir_matrix(:,1);
Spec_matrix(:,37) = Spec_matrix(:,1);
...
plot_angle = deg2rad(10*([1:37]-1));        % ATTENTION: theta should be in radians
...
polarplot(plot_angle,20*log10(abs(Spec_matrix(index_3,:)/(20*10^(-6)))),'--k','MarkerSize',3,'LineWidth',1);hold on;
freq_4 = 1500;  
...
polarplot(plot_angle,20*log10(abs(Spec_matrix(index_4,:)/(20*10^(-6)))),'--g','MarkerSize',3,'LineWidth',1);hold on;
freq_5 = 2000;  
...
rlim([-10 90]);
legend({strcat(num2str(faxis(index_1)),'Hz'),strcat(num2str(faxis(index_2)),'Hz'),strcat(num2str(faxis(index_3)),'Hz'),strcat(num2str(faxis(index_4)),'Hz'),strcat(num2str(faxis(index_5)),'Hz'),strcat(num2str(faxis(index_6)),'Hz'),strcat(num2str(faxis(index_7)),'Hz'),strcat(num2str(faxis(index_8)),'Hz'),strcat(num2str(faxis(index_9)),'Hz'),strcat(num2str(faxis(index_10)),'Hz'),});
...
plot_angle_colormap = [plot_angle(20:end),plot_angle(1:19)];
Spec_matrix_colormap = [Spec_matrix(:,20:end),Spec_matrix(:,1:19)];
[X,Y] = meshgrid(faxis,rad2deg(plot_angle_colormap));
surface(X,Y,20*log10(abs(Spec_matrix_colormap')/(20*10^(-6))),'FaceColor','interp','LineStyle','none');
colorbar
colormap jet
xlim([1/t_windur 20000]);  % ylim([0 360]);
caxis([20 80]);
xlabel('frequency (Hz)'); ylabel('direction (degrees)'); 
title('Frequency Spectrum Color Map');
...
for m = 1:3:36
    plot_angle = 30*(m-1);
    plot(faxis,20*log10(abs(Spec_matrix(:,m)/(20*10^(-6)))),'LineWidth',1);hold on;
end
grid on;
xlim([1/t_windur 20000]);  % ylim([min(20*log10(abs(P1/(20*10^(-6)))))-10 max(20*log10(abs(P1/(20*10^(-6)))))+10]);
xlabel('frequency (Hz)'); ylabel('magnitude (dB)'); 
legend({strcat('0',char(176)),strcat('30',char(176)),strcat('60',char(176)),strcat('90',char(176)),strcat('120',char(176)),strcat('150',char(176)),strcat('180',char(176)),strcat('210',char(176)),strcat('240',char(176)),strcat('270',char(176)),strcat('300',char(176)),strcat('330',char(176))},'Location','southwest');

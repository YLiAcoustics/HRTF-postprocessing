...
% data matrix option: Robert Baumgartner
    ...
    amt_disp('itdestimator','progress');

    % ---------------------- Applying low-pass ----------------------------

    if flags.do_lp
        amt_disp('Applying Butterworth low pass','progress')
        amt_disp(strcat('Polynomial order of Butterworth filter: '...
            ,num2str(kv.butterpoly)),'progress')
        amt_disp(strcat('Cut-off frequency is: '...
            ,num2str(kv.upper_cutfreq),' Hz'),'progress')
    ...
    switch(flags.mode)
        case 'Threshold'
            ...
            amt_disp(strcat('Threshold level is: ',...
                num2str(kv.threshlvl),'dB'),'progress')
    ...
    % ---------------------- Cross-Correlation ----------------------------        
        ...
        case 'MaxIACCr'
            amt_disp('MaxIACCr mode','progress')
            for ii=1:pos                
        ...
        case 'MaxIACCe'
            amt_disp('MaxIACCe mode','progress')
            for ii=1:pos
        ...
        case 'CenIACCr'
            amt_disp('CenIACCr mode','progress')
        ...
        case 'CenIACCe'
            amt_disp('CenIACCe mode','progress')
            x = transpose(1:(Ns*2-1));
            for ii=1:pos
                e_sir1 = envelope(squeeze(f_ir(ii,1,:)));
                e_sir2 = envelope(squeeze(f_ir(ii,2,:)));
        ...
        case 'CenIACC2e'
            amt_disp('CenIACC2e mode','progress')
            x = transpose(1:(Ns*2-1));
            for ii=1:pos              
                e_sir1 = envelope(squeeze(f_ir(ii,1,:)));
                e_sir2 = envelope(squeeze(f_ir(ii,2,:)));           
        ...
        case 'PhminXcor'
            amt_disp('PhminXcor mode','progress')
